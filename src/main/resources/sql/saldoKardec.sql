...
SELECT P.storeno, P.pdvno, P.xano, P.storeno AS loja, prdno, grade, date,
       CONCAT(P.nfno, '/', P.nfse) AS doc, (-(price) * 100 * qtty) AS cost, 'NF Saida' AS tipo,
       (-qtty * 1000) AS quant
...
WHERE N.status <> 1 AND date BETWEEN @DATA AND @DTREF AND N.cfo NOT IN (5922, 6922) AND
      (prdno = @PRD OR @PRD = '');
...
FROM sqldados.xaprd      AS P
  ...
  INNER JOIN sqldados.nf AS N
               ON P.storeno = N.storeno AND P.pdvno = N.pdvno AND P.xano = N.xano
WHERE date BETWEEN @DATA AND @DTREF AND (prdno = @PRD OR @PRD = '')
...
SELECT loja, prdno, grade, date, SUM(cost) AS cost, 'NF Saida' AS tipo, SUM(quant) AS quant
FROM PreSaida
...
SELECT X.storeno AS loja, X.prdno, X.grade, X.date, 'NF Cupom' AS tipo,
...
SELECT X.storeno AS loja, X.prdno, X.grade, X.date, 'Devolucao' AS tipo,
       SUM(-X.price * 100 * X.qtty) / 1000 AS cost, SUM(-X.qtty) AS quant, X.storeno, X.pdvno,
       X.xano
FROM xalog2        AS X
  LEFT JOIN  TXA
  ...
  INNER JOIN Lojas AS L
               ON L.storeno = X.storeno
WHERE icm_aliq & 4 = 0 AND X.xatype = 11 AND (X.doc LIKE 'DEVOL%' OR X.qtty > 0) AND
      TXA.xano IS NULL AND X.date BETWEEN @DATA AND @DTREF AND (prdno = @PRD OR @PRD = '')
GROUP BY X.storeno, prdno, grade, date;
...
SELECT storeno AS loja, prdno, grade, date, 'Mov Manual' AS tipo,
       SUM(/*cm_real*qtty/1000*/0) AS cost, SUM(qtty) AS quant
FROM sqldados.stkmov
  INNER JOIN Lojas
               USING (storeno)
WHERE qtty <> 0 AND MID(remarks, 36, 1) <> '1' AND date BETWEEN @DATA AND @DTREF AND
      (prdno = @PRD OR @PRD = '')
GROUP BY loja, prdno, grade, date;
...
SELECT I.storeno AS loja, P.prdno, P.grade, I.date AS date, 'NF Entrada' AS tipo,
       IF(type IN (0, 1, 2, 3), SUM(IF(cost4 = 0, IF(cost > fob * 10, 0, cost * 100),
                                       IF(cost4 > fob4 * 10, 0, cost4)) * qtty / 1000), 0) AS cost,
       SUM(qtty) AS quant
FROM sqldados.inv          AS I
  ...
  INNER JOIN sqldados.iprd AS P
               ON I.invno = P.invno
WHERE I.bits & POW(2, 4) = 0 AND I.auxShort13 & pow(2, 15) = 0 AND
      I.date BETWEEN @DATA AND @DTREF AND (P.prdno = @PRD OR @PRD = '') AND F.storeno IS NULL
...
SELECT loja AS storeno, prdno, grade, date, tipo, cost, quant
FROM Saldo
UNION ALL
SELECT loja AS storeno, prdno, grade, date, tipo, cost, quant
FROM NFCupom
...
SELECT storeno, prdno, grade, date, SUM(IF(cost > 0, cost, 0)) AS custo,
       SUM(IF(cost > 0, quant, 0)) AS qttyCusto, SUM(quant) AS qtty, 0 AS saldoAcumulado,
       0 AS custoAcumulado, 0 AS quantCustoAcu
FROM Kardec
...
FROM sqldados.stkchk
  INNER JOIN Lojas
               USING (storeno)
WHERE ym = @REF AND storeno IN (1, 2, 3, 4, 5, 6, 7, 10) AND (prdno = @PRD OR @PRD = '')
...
SELECT storeno, prdno, grade, SUM(qtty_varejo) AS quant
FROM sqldados.stk
  INNER JOIN Lojas
               USING (storeno)
...
SELECT storeno, prdno, grade, IFNULL(S.quant, 0) AS quantS, IFNULL(K.quant, 0) AS quantK
FROM COMP_MESTRE
...
SELECT LPAD(prdno * 1, 6, '0') AS codigo, grade AS grade, storeno AS loja,
       DATE_FORMAT(@REF * 100 + 1, '%m/%Y') AS mes_ano, quantS / 1000 AS saldoEstoque,
       quantK / 1000 AS saldoKardec, (quantS - quantK) / 1000 AS diferecenca
FROM COMP
WHERE quantS <> quantK
