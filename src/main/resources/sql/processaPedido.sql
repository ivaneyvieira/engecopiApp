...
SELECT E.storeno, E.ordno, E.prdno, E.grade, qtty,
       ...
       CAST(CONCAT(F.no, ' ', MID(F.sname, 1, 4)) AS CHAR) AS frabricante, C.no AS custno, E.empno
...
SELECT @INVNO AS invno, vendno, ordno, 0 AS xfrno, current_date * 1 AS issue_date,
       ...
       0 AS auxMoney9, 0 AS auxMoney10, 0 AS auxMoney11, 0 AS auxMoney12, 0 AS auxMoney13, 0 AS l1,
       0 AS l2, 0 AS l3, 0 AS l4, 0 AS l5, current_date * 1 AS l6, 0 AS l7, 0 AS l8, 0 AS m1,
       0 AS m2, 0 AS m3, 0 AS m4, 0 AS m5, 0 AS m6, 0 AS m7, 0 AS m8, 0 AS weight, 0 AS carrno,
       0 AS packages, storeno, 0 AS indxno, 9 AS book_bits, 8 AS type, 1 AS usernoFirst,
       1 AS usernoLast, 0 AS nfStoreno, 3 AS bits, 0 AS padbyte, storeno AS auxShort1,
       0 AS auxShort2, 0 AS auxShort3, 0 AS auxShort4, 0 AS auxShort5, 0 AS auxShort6,
       ...
       0 AS s6, 0 AS s7, 0 AS s8, @NFNO AS nfname, @SERIE AS invse, 2 AS account,
...
SELECT @XANO AS xano, @NFNO AS nfno, custno, current_date * 1 AS issuedate,
       ...
       CAST(CONCAT(@OBS, ' ', GROUP_CONCAT(DISTINCT frabricante ORDER BY frabricante SEPARATOR
                                           ' ')) AS CHAR) AS remarks, '' AS padbyte,
       '' AS print_remarks, '' AS remarksCancel, @DOC AS c1, '' AS c2, '' AS wshash
FROM T
WHERE @TIPO = 'S'
GROUP BY storeno, ordno;
/*
